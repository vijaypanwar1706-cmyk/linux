...
static bool
wm8960_volatile (struct device *dev, unsigned int reg)
{
...
static int
wm8960_set_deemph (struct snd_soc_component *component)
{
    printk(KERN_INFO "%s:%s(): reached here\n", __FILE__, __func__);
    ; /* avoid -Wswitch-unreachable */
...
static int
wm8960_get_deemph (struct snd_kcontrol *kcontrol,
...
static int
wm8960_put_deemph (struct snd_kcontrol *kcontrol,
                   struct snd_ctl_elem_value *ucontrol)
{
    printk(KERN_INFO "%s:%s(): reached here\n", __FILE__, __func__);
    ; /* avoid -Wswitch-unreachable */
...
static const SNDRV_CTL_TLVD_DECLARE_DB_RANGE (micboost_tlv,
                                              0, 1, TLV_DB_SCALE_ITEM (0,
                                                                       1300,
                                                                       0), 2,
                                              3, TLV_DB_SCALE_ITEM (2000, 900,
                                                                    0),);
...
static int
wm8960_add_widgets (struct snd_soc_component *component)
{
    printk(KERN_INFO "%s:%s(): reached here\n", __FILE__, __func__);
    ; /* avoid -Wswitch-unreachable */
    struct wm8960_priv *wm8960 = snd_soc_component_get_drvdata (component);
...
static int
wm8960_set_dai_fmt (struct snd_soc_dai *codec_dai, unsigned int fmt)
{
    printk(KERN_INFO "%s:%s(): reached here\n", __FILE__, __func__);
    ; /* avoid -Wswitch-unreachable */
 ...
 */
static int
wm8960_configure_sysclk (struct wm8960_priv *wm8960, int mclk,
 ...
 */
static int
wm8960_configure_pll (struct snd_soc_component *component, int freq_in,
                      int *sysclk_idx, int *dac_idx, int *bclk_idx)
{
    printk(KERN_INFO "%s:%s(): reached here\n", __FILE__, __func__);
    ; /* avoid -Wswitch-unreachable */
...
static int
wm8960_configure_clocking (struct snd_soc_component *component)
{
    printk(KERN_INFO "%s:%s(): reached here\n", __FILE__, __func__);
    ; /* avoid -Wswitch-unreachable */
    struct wm8960_priv *wm8960 = snd_soc_component_get_drvdata (component);
    ...
    printk (KERN_INFO
            "[vijayp][clk] iface1=0x%x sysclk=%d clk_id=%d freq_in=%d\n",
    ...
    if (!(iface1 & (1 << 6)) && !wm8960->sysclk) {
        ...
        printk (KERN_WARNING
                "[vijayp][clk] Slave mode but no SYSCLK → skipping clock config\n");
        return 0;
    }
    ...
    if (wm8960->clk_id == WM8960_SYSCLK_AUTO) {
        ...
        printk (KERN_INFO
                "[vijayp][clk] SYSCLK MODE = AUTO → Disable PLL, use MCLK\n");
        ...
        printk (KERN_INFO
                "[vijayp][clk] SYSCLK explicitly set: freq_out = %d Hz\n",
                freq_out);
    }
    else {
    ...
    if (wm8960->clk_id != WM8960_SYSCLK_PLL) {
        printk (KERN_INFO
                "[vijayp][clk] Trying direct SYSCLK configuration...\n");
        ret = wm8960_configure_sysclk (wm8960, freq_out, &i, &j, &k);
        if (ret >= 0) {
            printk (KERN_INFO
                    "[vijayp][clk] Direct SYSCLK config OK: i=%d j=%d k=%d\n",
                    i, j, k);
            ...
            printk (KERN_ERR
                    "[vijayp][clk] ERROR: Direct SYSCLK configuration failed\n");
            return -EINVAL;
        }
        printk (KERN_INFO
                "[vijayp][clk] Direct config failed but AUTO mode → Trying PLL\n");
    }
    ...
    printk (KERN_INFO
            "[vijayp][clk] PLL configured OK: freq_out=%d i=%d j=%d k=%d\n",
            freq_out, i, j, k);
    wm8960_set_pll (component, freq_in, freq_out);

  configure_clock:
    printk (KERN_INFO
            "[vijayp][clk] Applying SYSCLK: CLOCK1(bits 1-2) = %d\n", i);
    printk (KERN_INFO
            "[vijayp][clk] Applying LRCLK dividers j=%d (CLOCK1 bits 3-5 and 6-8)\n",
            j);
    printk (KERN_INFO
            "[vijayp][clk] Applying BCLK divider k=%d (CLOCK2 bits 0-3)\n",
            k);
    ...
    printk (KERN_INFO
            "[vijayp][clk] EXIT wm8960_configure_clocking() SUCCESS\n");

    return 0;
}

static int
wm8960_hw_params (struct snd_pcm_substream *substream,
                  struct snd_pcm_hw_params *params, struct snd_soc_dai *dai)
{
    ...
    printk (KERN_INFO "[vijayp] substream=%px stream=%d (%s)\n",
            substream, substream->stream, tx ? "PLAYBACK" : "CAPTURE");

    printk (KERN_INFO
            "[vijayp] params: rate=%d Hz, channels=%d, width=%d bits\n",
    ...
    printk (KERN_INFO "[vijayp] stream_in_use: playback=%d capture=%d\n",
            wm8960->is_stream_in_use[1], wm8960->is_stream_in_use[0]);
...
static int
wm8960_hw_free (struct snd_pcm_substream *substream, struct snd_soc_dai *dai)
{
    printk(KERN_INFO "%s:%s(): reached here\n", __FILE__, __func__);
    ; /* avoid -Wswitch-unreachable */
    struct snd_soc_component *component = dai->component;
...
static int
wm8960_mute (struct snd_soc_dai *dai, int mute, int direction)
{
    printk(KERN_INFO "%s:%s(): reached here\n", __FILE__, __func__);
    ; /* avoid -Wswitch-unreachable */
    struct snd_soc_component *component = dai->component;
...
static int
wm8960_set_bias_level_out3 (struct snd_soc_component *component,
                            enum snd_soc_bias_level level)
{
    printk(KERN_INFO "%s:%s(): reached here\n", __FILE__, __func__);
    ; /* avoid -Wswitch-unreachable */
    ...
    case SND_SOC_BIAS_STANDBY:
        if (snd_soc_component_get_bias_level (component) == SND_SOC_BIAS_OFF) {
            ...
            tout =
                WM8960_DSCH_TOUT - ktime_ms_delta (ktime_get (),
                                                   wm8960->dsch_start);
            if (tout > 0)
                msleep (tout);
            ...
            snd_soc_component_update_bits (component, WM8960_POWER1, 0x80,
                                           0x80);
            msleep (100);
...
static int
wm8960_set_bias_level_capless (struct snd_soc_component *component,
                               enum snd_soc_bias_level level)
{
    printk(KERN_INFO "%s:%s(): reached here\n", __FILE__, __func__);
    ; /* avoid -Wswitch-unreachable */
...
static bool
is_pll_freq_available (unsigned int source, unsigned int target)
{
    printk(KERN_INFO "%s:%s(): reached here\n", __FILE__, __func__);
    ; /* avoid -Wswitch-unreachable */
    unsigned int Ndiv;
...
static int
pll_factors (unsigned int source, unsigned int target,
...
static int
wm8960_set_pll (struct snd_soc_component *component,
                unsigned int freq_in, unsigned int freq_out)
{
    printk(KERN_INFO "%s:%s(): reached here\n", __FILE__, __func__);
    ; /* avoid -Wswitch-unreachable */
...
static int
wm8960_set_dai_pll (struct snd_soc_dai *codec_dai, int pll_id,
...
static int
wm8960_set_dai_clkdiv (struct snd_soc_dai *codec_dai, int div_id, int div)
{
    printk(KERN_INFO "%s:%s(): reached here\n", __FILE__, __func__);
    ; /* avoid -Wswitch-unreachable */
    struct snd_soc_component *component = codec_dai->component;
...
static int
wm8960_set_bias_level (struct snd_soc_component *component,
                       enum snd_soc_bias_level level)
{
    printk(KERN_INFO "%s:%s(): reached here\n", __FILE__, __func__);
    ; /* avoid -Wswitch-unreachable */
...
static int
wm8960_set_dai_sysclk (struct snd_soc_dai *dai, int clk_id,
...
static int
wm8960_probe (struct snd_soc_component *component)
{
    printk(KERN_INFO "%s:%s(): reached here\n", __FILE__, __func__);
    ; /* avoid -Wswitch-unreachable */
    struct wm8960_priv *wm8960 = snd_soc_component_get_drvdata (component);
...
static void
wm8960_set_pdata_from_of (struct i2c_client *i2c, struct wm8960_data *pdata)
{
    printk(KERN_INFO "%s:%s(): reached here\n", __FILE__, __func__);
    ; /* avoid -Wswitch-unreachable */
...
static void
wm8960_i2c_remove (struct i2c_client *client)
{
    printk(KERN_INFO "%s:%s(): reached here\n", __FILE__, __func__);
    ; /* avoid -Wswitch-unreachable */
